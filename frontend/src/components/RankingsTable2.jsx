          ...
          {sortedData.map((item, idx) => {
            ...
            return (
              <div
                ...
                className={`p-4 space-y-4 rounded-4xl inset-shadow-sm shadow-md border border-black/5 bg-white/50 cursor-pointer transform transition-transform duration-200 ${(isCountry || (!isCountry && item.bucket === "Best Match")) ? "flex flex-col" : "hidden"}`}
                role="button"
                tabIndex={0}
              ...
              >
                <div className="flex justify-between">
                  ...
                  <div className="flex gap-2 flex-wrap justify-end">
                    <div
                      className={`flex items-center gap-1.5 rounded-full h-7 px-2.5 ${isCountry && item.classes} ${!isCountry && "bg-black/5 text-black/80 ring-2 ring-black/10"}`}
                      aria-label={`${item.label} (${item.grade})`}
                    >
                      {isCountry && <span className="text-xs font-medium">{item.bucket}</span>}
                      <div className={`flex justify-center items-center w-6 h-6 rounded-full font-semibold text-xs ${isCountry && "bg-white/50 ring ring-white/30"}`}>{item.grade}</div>
                      {/* <span className="text-xs font-medium">{item.final_score}</span> */}
                    </div>
                    {/* <button onClick={(e) => {
                      e.stopPropagation();
                      if (shortlistedCountries.includes(item[nameKey])) {
                        setShortlistedCountries((prev) =>
                          prev.filter(country => country !== item[nameKey])
                        );
                      }
                      else {
                        setShortlistedCountries((prev) =>
                    ...
                    >
                      <Heart fill={shortlistedCountries.includes(item[nameKey]) ? "rgba(225, 29, 72, 0.8)" : "transparent"} className={`w-4 h-4 transition-all duration-300 group-active:scale-120 ${shortlistedCountries.includes(item[nameKey]) ? "text-rose-600/80" : "text-black/80"}`} />
                    </button> */}
                  </div>
                </div>
