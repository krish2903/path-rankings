...
const ItemSelector = ({
  ...
  const toggleItem = (name) => {
    if (selectedItems.includes(name)) {
      setSelectedItems(selectedItems.filter((d) => d !== name));
        ...
        {items.map((name) => {
          ...
          return (
            <button
              ...
              className="relative cursor-pointer flex flex-col items-center focus:outline-none disabled:cursor-not-allowed"
              disabled={!selected && selectedItems.length >= maxSelection}
              aria-pressed={selected}
              aria-label={`${name} ${selected ? "selected" : "not selected"}`}
              title={name}
            >
              <div
                className={`relative flex items-center justify-center h-16 w-16 md:h-20 md:w-20 rounded-full border-3 transition-colors bg-white ${selected ? "border-orange-500" : "border-black/10"
                  }`}
              >
                {Icon && (
                  <Icon
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-6 justify-center w-full max-w-xs sm:max-w-none">
          <button
            className="bg-gray-200 hover:bg-gray-300 text-black/80 font-semibold py-2 sm:py-3 rounded-full text-sm sm:text-base md:text-lg transition cursor-pointer w-full sm:w-32"
